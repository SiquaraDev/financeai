 ...
 * useReportsData
 *
 * Fetches and aggregates monthly and category data for the reports page.
 * Extracted from `reports/page.tsx` to keep the page component clean.
 *
 * @param period - "monthly" (6 months) | "quarterly" (12 months) | "yearly" (24 months)
 */
...
import type { MonthData, CategoryData } from "@/components/reports/ChartRenderer";
...
export function useReportsData(period: PeriodType): UseReportsDataReturn {
  ...
  const fetchData = useCallback(async () => {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      results.push({
        month: format(date, period === "monthly" ? "MMM/yy" : "MM/yyyy", {
          locale: ptBR,
        }),
